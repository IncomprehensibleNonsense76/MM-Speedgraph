...
def check(
    ...
    time: frozenset[T] | None = N,
    duration: int = 0,
    warp_to: str | None = None,
) -> Check:
    c = Check(id, scene, requires, time, duration, warp_to)
    CHECKS[id] = c
    return c
...
check(
    I.BottleAliens, S.RomaniRanch, {I.Bow, I.Epona}, N1
)  # aliens attack ~2:30 AM Night 1
check(I.GoldDust, S.GoronRacetrack, {M.Goron, R.Goht})
check(
    I.BottleGoldDust, S.WestClockTown, {I.GoldDust}, ANY_NIGHT
)  # Curiosity Shop after 10 PM
...
check(
    WFT_SK1, room_node(_WFT, 5), {Songs.Sonata, M.Deku, I.Magic}
)  # small key in Room 5
...
check(
    ...
    {I.BossKeyWFT},
    duration=180,
    warp_to=S.DekuPrincessPrison,
)  # 3 min: boss -> GC -> prison
check(
    Songs.OathToOrder, S.DekuPrincessPrison, {R.Odolwa}
)  # learned in GC during boss CS
...
check(
    M.Scents, S.DekuPalace, {I.DekuPrincess, M.Deku}
)  # release princess -> butler race
check(
    M.Truth, S.SwampSpiderHouse, {M.Deku, I.Bottle, I.Bow}
)  # need bow (or stick) to enter, all 30 skulltulas
check(I.SpinAttack, S.Woodfall, {R.Odolwa})

# =====================
# === MOUNTAIN / SNOWHEAD
# =====================
check(I.LensOfTruth, S.LonePeakShrine, {I.Magic})  # bow+bombs for mtn access is on edge
check(
    M.Goron, S.MountainVillage, {I.LensOfTruth, Songs.Healing}, duration=90
)  # 90s cutscene
...
check(
    R.Goht, S.SnowheadTemple, {I.BossKeySHT}, duration=120, warp_to=S.MountainVillage
)  # 2 min: boss -> GC -> mtn village
...
# =====================
check(
    M.Zora, S.GreatBayCoast, {Songs.Healing}, duration=90
)  # 90s CS; Epona/Goron+Bombs for GBC access is on edge
check(
    I.Hookshot, S.PiratesFortress, {M.Zora, M.Goron}
)  # Zora to swim, Goron for sewers, Deku/Bow for beehive
check(
    Songs.NewWave,
    S.MarineResearchLab,
    {M.Zora, I.Hookshot, I.Bottle, I.BottleAliens, I.BottleGoldDust},
)
check(I.IceArrows, S.GreatBayTemple, {Songs.NewWave, M.Zora})
check(I.BossKeyGBT, S.GreatBayTemple, {I.IceArrows})
check(
    R.Gyorg, S.GreatBayTemple, {I.BossKeyGBT}, duration=120, warp_to=S.ZoraCape
)  # 2 min: boss -> GC -> zora cape
...
check(
    M.Captain, S.IkanaGraveyard, {Songs.Sonata}
)  # Garo+Hookshot for canyon access is on edge
check(Songs.Storms, S.IkanaGraveyard, {M.Captain})
check(M.Gibdo, S.IkanaCanyon, {Songs.Storms, Songs.Healing})
check(
    I.MirrorShield,
    S.BeneathTheWell,
    {M.Gibdo, I.BombBag, I.Bottle, I.MagicBeans, I.Epona},
)
...
check(
    R.Twinmold,
    S.StoneTowerTemple,
    {I.BossKeySTT, M.Giant},
    duration=120,
    warp_to=S.IkanaCanyon,
)  # 2 min: boss -> GC -> ikana canyon
...
check(
    E.EnterMoon,
    S.ClockTowerRooftop,
    {R.Odolwa, R.Goht, R.Gyorg, R.Twinmold, Songs.OathToOrder, I.Ocarina},
    N3,
)
...
# =====================
check(
    I.LetterToKafei, S.EastClockTown, {M.KafeiMask, I.RoomKey}, N1
)  # midnight meeting Night 1
...
check(M.Couple, S.EastClockTown, {I.PendantOfMemories, M.Garo, I.Hookshot}, N3)
